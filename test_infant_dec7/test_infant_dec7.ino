...
#define sfm3300i2c 0x40
 ...
 #define NO_BREATH 0 // for variable breath type
 #define SMALL_BREATH 1
 #define MED_BREATH 2
 #define LARGE_BREATH 3


#define ADULT_MALE_MASS 90.6; // kg
#define ADULT_FEMALE_MASS 77.5; // kg
#define INFANT_MASS 3.5; // kg
...
//  digitalWrite(5, HIGH); //green on
  ...
  colorSet(strip.Color(0, 16,   0), 0); // Set color to green at start
...
float rev_flow_t = 4294967295; // set to high value so when delta_t is measured it is always negative
float new_breath_t = 4294967295; // set to high value so when delta_t is measured it is always negative
float blink_start_t = 4294967295; // set to high value so when delta_t is measured it is always negative

float breath_time = 1000; // milliseconds that the breath should last for (change for infants; impacts line 292)

float mass = INFANT_MASS; // specifies mass based on patient

float tvol_low_thresh = 8.0*mass; // (8 mL/kg)*(mass kg)
float tvol_high_thresh = 10.0*mass; // (10 mL/kg)*(mass kg)
...
int breath_type = NO_BREATH; //1,2,3 #define
...
void SFM_measure(float ms_curr) {
...
//    Serial.println("I've missed a spot");
  ...
  if (((float) vol >= (float) tvol_high_thresh) && ((float) vol_prev < (float) tvol_high_thresh)) { // when the volume passes the higher tidal volume threshold
      digitalWrite(10, HIGH); // tell solenoid to turn on
      breath_type = LARGE_BREATH;
//      Serial.println("5");
  }

  
  if (((float) ms_curr - (float) rev_flow_t) >= 1000) { // waits one second until solenoid shut off

      digitalWrite(10, LOW); // tell solenoid to turn off
  }
  
  if ((ms_curr - new_breath_t > 0) && (ms_curr - rev_flow_t > 0) && (breath_type == MED_BREATH || breath_type == LARGE_BREATH)) { 

//    Serial.println("6");
    if ((ms_curr - new_breath_t < breath_time)){ // waits six seconds until it resets green lights
      blinkRed(ms_curr, blink_start_t);
      ...
      blink_start_t = 4294967295;
      colorSet(strip.Color(0, 16,   0), 0); // Green
      vol = 0;
      breath_type = NO_BREATH;
    }
...
void blinkRed(float ms_curr, float blink_start_t){

 if ( (((int)floor((ms_curr - blink_start_t)/200.0) % 2)) == 0 ){
