...
float breath_time = 6000; // milliseconds that the breath should last for (change for infants; impacts line 292)

float mass = ADULT_FEMALE_MASS; // specifies mass based on patient

//float tvol_low_thresh = 8.0*mass; // (8 mL/kg)*(mass kg)
//float tvol_high_thresh = 10.0*mass; // (10 mL/kg)*(mass kg)

float tvol_low_thresh = 350;
float tvol_high_thresh = 400; // test
...
void blinkRed(float ms_curr, float blink_start_t){
 ...
 if ( (((int)floor((ms_curr - blink_start_t)/500.0) % 2)) == 0 ){
