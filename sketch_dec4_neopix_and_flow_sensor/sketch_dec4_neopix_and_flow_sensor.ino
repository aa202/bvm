...
const unsigned mms = 1; // measurement interval in ms
...
void SFM_measure(unsigned long ms_curr) {
  ...
  if (3 == Wire.requestFrom(sfm3300i2c, 3)) {
    ...
    if (flow_sp != flow_sign) { // once the flow changed direction
      ...
      if (vol_prev < tvol_low_thresh && vol_prev > 5 && (small_breath == true)){ // turns lights back on if volume is not enough
          rev_flow_t = millis();    // saves time of reversal of flow

//          Serial.println("8");

          pixel = ceil(12*(vol/tvol_low_thresh)); // current last pixel that should be off
...
//    Serial.println("6");
  ...
  else if (ms_curr - rev_flow_t >= 4000) { // waits four seconds until it resets green lights
